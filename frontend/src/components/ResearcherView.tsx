...
export function ResearcherView({ session }: { session: SessionView }) {
  ...
  return (
    <section className="panel" aria-labelledby="researcher-title">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="eyebrow">RESEARCHER VIEW</p>
          <h2 id="researcher-title" className="panel-title">Question utility audit</h2>
        </div>
        <p className="text-xs text-slate-500">Uniform synthetic branches · learned prior 없음</p>
      </div>
          ...
          return (
            <article key={candidate.question_id} className="rounded-xl border border-slate-800 bg-slate-950/50 p-4">
              ...
              <dl className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-400">
                <div><dt>Final utility</dt><dd className="mt-1 font-mono text-white">{utility?.final_utility.toFixed(3) ?? "—"}</dd></div>
                <div><dt>Mean risk ↓</dt><dd className="mt-1 font-mono text-white">{utility ? `${Math.round(utility.mean_risk_reduction * 100)}%` : "—"}</dd></div>
                <div><dt>Affected criteria</dt><dd className="mt-1 font-mono text-white">{candidate.affected.length}</dd></div>
                <div><dt>Slot dedup</dt><dd className="mt-1 font-mono text-white">1 action</dd></div>
              </dl>
            </article>
          );
        })}
